...
select max(total_laid_off), max(percentage_laid_off)	-- Finding out the highest layoff occured in a day + highest layoff percentage
from layoffs_staging2;

select * 
from `layoffs_staging2`
where percentage_laid_off = 1
order by total_laid_off desc;	-- Finding out all the companies that went under with the one with the most layoffs at the top

select company, sum(total_laid_off)
from `layoffs_staging2`
group by company;	-- Finding out which company had the greatest layoff number across all years
